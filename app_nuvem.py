...
# Função de validação de Ano (Formato 20XX)
...
# --- 1. ABA PESQUISAR E GERIR (COM EDITAR E EXCLUIR) ---
with aba_pesquisa:
    ...
    if len(selecao.selection.rows) > 0:
        ...
        with st.form("form_edicao"):
            ...
            # Ano (Validação)
            e_ano = c2.text_input("Ano (20XX)", value=str(row.get('Ano', '')))
            
            # Banca (Menu Suspenso)
            b_at = str(row.get('Banca', ''))
            ...
            if btn_save:
                if not validar_ano(e_ano):
                    st.error("⚠️ Erro: Selecione uma data válida (Formato 20XX)")
                else:
            ...
            if btn_del:
                ...
                guardar_dados_nuvem(df, "Banco")
                st.warning("Questão excluída!")
                st.rerun()

# --- 2. ABA ADICIONAR NOVA (COM VALIDAÇÕES) ---
with aba_add:
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        n_ano = c3.text_input("Ano (Ex: 2024)")
...
with aba_rel:
    ...
    m3.metric("Bancas", df['Banca'].nunique())
    m4.metric("Anos", df['Ano'].nunique())
    ...
    with col_g2:
        ...
        st.bar_chart(df['Banca'].value_counts())

    st.markdown("---")
    
    # 2.2 Top 5 Assuntos por Matéria
    st.write("#### 🎯 Top 5 Assuntos de Cada Matéria")
    for mat in LISTA_MATERIAS:
        assuntos = df[df['Matéria'] == mat]['Assunto'].value_counts().head(5)
    ...
    # 2.4 Últimas Adições/Alterações
    with col_h2:
        st.write("#### 🕒 5 Últimas Alterações/Adições")
        if not df.empty:
            # Ordena por modificação (precisa converter string para datetime para ordenar certo)
            df_recent = df.copy()
...
with aba_import:
    ...
    # ... (mesmo código de importação anterior)
    up = st.file_uploader("Arquivo CSV", type="csv")
    if up and st.button("Importar Dados"):
        old = pd.read_csv(up, dtype={'Código': str})
        # Lógica de merge e upload aqui
